...
**Primary tools:** Atlassian MCP tools (`createJiraIssue`, `getVisibleJiraProjects`, `getJiraProjectIssueTypesMetadata`)
...
**Ask:** "Which Jira project should I create these tickets in? (e.g., PROJ, ENG)"

If user is unsure, call `getVisibleJiraProjects` via MCP to show options.

Then call `getJiraProjectIssueTypesMetadata` to identify available issue types:
```
...
**For 1-4 tickets:** Use MCP `createJiraIssue` for each:
```
createJiraIssue(
  cloudId="...",
  projectKey="PROJ",
...
**For 5+ tickets:** Use batch script for parallel creation:
```bash
echo '[
  {"summary": "Implement user registration API", "description": "..."},
  {"summary": "Build login form UI", "description": "..."},
  ...
]' | uv run python scripts/batch/batch_create.py --stdin --type story --parent PROJ-123
```
